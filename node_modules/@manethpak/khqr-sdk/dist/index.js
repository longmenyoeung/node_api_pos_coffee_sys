...
__export(src_exports, {
  createKHQR: () => createKHQR
});
module.exports = __toCommonJS(src_exports);

// src/qr/core/generate.ts
var import_crypto = require("crypto");

// src/qr/constants/emv.ts
var EMV_TAGS = {
  // Standard EMV tags
...
// src/qr/helper/tags.ts
function formatTag(tag2, value) {
  const length = value.length.toString().padStart(2, "0");
  return `${tag2}${length}${value}`;
}
function parseTag(data, offset = 0) {
  if (data.length < offset + 4) {
    throw new Error("Invalid tag format: insufficient length");
  }
  const tag2 = data.substring(offset, offset + 2);
...
function createPayloadFormatIndicator() {
  return formatTag(
    EMV_TAGS.PAYLOAD_FORMAT_INDICATOR,
    EMV_DEFAULTS.PAYLOAD_FORMAT_INDICATOR
  );
}
function createPointOfInitiationMethod(isStatic) {
  const value = isStatic ? QR_TYPE_INDICATORS.STATIC_QR : QR_TYPE_INDICATORS.DYNAMIC_QR;
  return formatTag(EMV_TAGS.POINT_OF_INITIATION_METHOD, value);
}
function createMerchantAccountInformation(type, info) {
  const tag2 = type === "individual" ? EMV_TAGS.MERCHANT_ACCOUNT_INFORMATION_INDIVIDUAL : EMV_TAGS.MERCHANT_ACCOUNT_INFORMATION_MERCHANT;
  let subTags = "";
  subTags += formatTag(EMV_TAGS.BAKONG_ACCOUNT_IDENTIFIER, info.bakongAccountID);
  if (type === "merchant") {
    const merchantInfo = info;
  ...
  return formatTag(tag2, subTags);
}
function createUnionpayMerchantAccount(accountInfo) {
  return formatTag(EMV_TAGS.UNIONPAY_MERCHANT_ACCOUNT, accountInfo);
}
function createMerchantCategoryCode(categoryCode) {
  const code = categoryCode || EMV_DEFAULTS.MERCHANT_CATEGORY_CODE;
  return formatTag(EMV_TAGS.MERCHANT_CATEGORY_CODE, code);
}
function createTransactionCurrency(currency = "KHR") {
  const currencyCode = currency === "KHR" ? CURRENCY_CODES.KHR : CURRENCY_CODES.USD;
  return formatTag(EMV_TAGS.TRANSACTION_CURRENCY, currencyCode);
}
function createTransactionAmount(amount, currency) {
  let formattedAmount;
  if (currency === "KHR") {
...
function createLanguageTemplate(languageInfo) {
  let subTags = "";
  ...
  return subTags ? formatTag(EMV_TAGS.MERCHANT_INFORMATION_LANGUAGE_TEMPLATE, subTags) : "";
}
function createTimestamp(creationTimestamp, expirationTimestamp) {
  let subTags = "";
  subTags += formatTag(
...
function createCRCTag(crcValue) {
...
function calculateCRC16(input) {
  ...
  const finalCheckSum = (crc ^ 0) & 65535;
  return finalCheckSum.toString(16).toUpperCase().padStart(4, "0");
}

// src/qr/helper/validator.ts
var validators = {
  /**
   ...
   */
  expirationTimestamp: (timestamp, qrType) => {
    if (qrType === "dynamic") {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  isMerchantInfo: (info) => {
    return info && typeof info === "object" && "merchantID" in info && typeof info.merchantID === "string";
  },
  /**
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  isIndividualInfo: (info) => {
    return info && typeof info === "object" && "bakongAccountID" in info && typeof info.bakongAccountID === "string";
  },
  /**
   ...
   */
  isDynamicQR: (qrString) => {
    if (!qrString) return false;
    return /^000201010212/.test(qrString);
  }
};

// src/qr/helper/errors.ts
var ERROR_CODES = {
...
var KHQRError = class extends Error {
...
var error = {
  invalidQR: (message = "Invalid QR code format", details) => new KHQRError(ERROR_CODES.INVALID_QR, message, details),
  invalidAmount: (message = "Invalid amount for currency", details) => new KHQRError(ERROR_CODES.INVALID_AMOUNT, message, details),
  invalidAccount: (message = "Invalid account information", details) => new KHQRError(ERROR_CODES.INVALID_ACCOUNT, message, details),
  requiredField: (field, details) => new KHQRError(
    ERROR_CODES.REQUIRED_FIELD,
    `Field '${field}' is required`,
    details ?? { field }
  ),
  invalidFormat: (fields, details) => new KHQRError(
    ERROR_CODES.INVALID_FORMAT,
    `Invalid or Missing fields '${fields}'`,
    details
  ),
  invalidCRC: (details) => new KHQRError(ERROR_CODES.CRC_INVALID, "CRC checksum is invalid", details),
  invalidQRString: (details) => new KHQRError(
...
// src/qr/helper/result.ts
var success = (value) => ({
  result: value,
  error: null
});
function failed(errorOrMessage, details, code = ERROR_CODES.INVALID_FORMAT) {
  const err = errorOrMessage instanceof KHQRError ? errorOrMessage : new KHQRError(code, errorOrMessage, details);
  return {
    result: null,
    error: err
  };
...
// src/qr/core/generate.ts
function generateKHQR(info) {
  try {
    const accountType = validators.isMerchantInfo(info) ? "merchant" : "individual";
    const validation = accountType === "merchant" ? validators.validateMerchantInfo(info) : validators.validateIndividualInfo(info);
    if (!validation.isValid) {
      return failed(error.invalidFormat(validation.errors.join(", ")));
    }
    const currency = info.currency || "KHR";
    ...
    if (info.upiMerchantAccount) {
      ...
      qrData = beforeUPI + tag.unionpayMerchantAccount(info.upiMerchantAccount) + afterUPI;
    }
    ...
    const md5Hash = (0, import_crypto.createHash)("md5").update(qrData).digest("hex");
    return success({
      qr: qrData,
      md5: md5Hash
    });
...
function parseSubtags(tagValue, parentTag) {
  const result = {};
  ...
  while (remainingValue.length > 0) {
    try {
      ...
      if (mapping) {
        const fieldName = parentTag === "30" && mapping.name === "accountInformation" ? "merchantID" : mapping.name;
        if (fieldName === "creationTimestamp" || fieldName === "expirationTimestamp") {
...
function decodeKHQR(qrString) {
  try {
    if (!qrString || typeof qrString !== "string") {
      return failed(error.invalidQRString());
    }
    const parsedTags = {};
    let remainingQR = qrString;
    let lastTag = "";
    let _merchantType = null;
    while (remainingQR.length > 0) {
      try {
        ...
        } else if (currentTag === EMV_TAGS.MERCHANT_ACCOUNT_INFORMATION_INDIVIDUAL) {
          _merchantType = EMV_TAGS.MERCHANT_ACCOUNT_INFORMATION_INDIVIDUAL;
        }
        const validTags = Object.values(EMV_TAGS);
        ...
        return failed(error.invalidFormat("qrString", details));
      }
    }
    const result = {};
    Object.entries(parsedTags).forEach(([tag2, value]) => {
      switch (tag2) {
        ...
        case EMV_TAGS.MERCHANT_ACCOUNT_INFORMATION_MERCHANT: {
          const subtagData = parseSubtags(value, tag2);
        ...
        case EMV_TAGS.ADDITIONAL_DATA_TAG:
          result.additionalData = parseSubtags(value, tag2);
          break;
        case EMV_TAGS.MERCHANT_INFORMATION_LANGUAGE_TEMPLATE:
          result.languageTemplate = parseSubtags(value, tag2);
          break;
        case EMV_TAGS.TIMESTAMP_TAG: {
          const timestampData = parseSubtags(value, tag2);
          if (timestampData.creationTimestamp || timestampData.expirationTimestamp) {
    ...
    const details = err instanceof Error ? { cause: err.message } : err ? { cause: String(err) } : void 0;
    return failed(error.invalidFormat("qrString", details));
  }
}

// src/qr/core/verify-string.ts
function verifyKHQRString(qrString) {
  try {
    if (!qrString || typeof qrString !== "string") {
      return failed(error.invalidQRString());
    }
...
// src/fetch/create-fetch.ts
var FetchAPI = class {
  constructor(option) {
    this.option = { baseURL: "", authToken: "" };
   ...
   */
  async check_backong_account(accountId) {
    validator(accountId).bakongId().min(3).max(50).validate();
    return this.post("/v1/check_bakong_account", {
      accountId
    });
  }
  async generate_deeplink(input) {
    return this.post(
      "/v1/generate_deeplink_by_qr",
      input
    );
  }
  async check_transaction_by_md5(md5) {
    return this.post("/v1/check_transaction_by_md5", {
      md5
    });
  }
  async check_transaction_by_hash(hash) {
    return this.post("/v1/check_transaction_by_hash", {
      hash
    });
  }
  async check_transaction_by_short_hash(shortHash) {
    return this.post(
      "/v1/check_transaction_by_short_hash",
      shortHash
    );
  }
  async check_transaction_by_instruction_ref(instructionRef) {
    return this.post(
      "/v1/check_transaction_by_instruction_ref",
      { instructionRef }
    );
  }
  async check_transaction_by_external_ref(externalRef) {
    return this.post(
      "/v1/check_transaction_by_external_ref",
      { externalRef }
    );
  }
  async check_transaction_by_md5_list(md5List) {
    return this.post(
      "/v1/check_transaction_by_md5_list",
      { md5List }
    );
  }
  async check_transaction_by_hash_list(hashList) {
    return this.post(
...
var createFetch = (option) => new FetchAPI(option);
var create_fetch_default = createFetch;

// src/index.ts
var createKHQR = (fetchOption) => {
  return {
    api: create_fetch_default(fetchOption),
    qr
  };
};
